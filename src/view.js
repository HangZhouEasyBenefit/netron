...
view.ModelFactoryService = class {
    ...
    open(context) {
        return new Promise((resolve, reject) => {
            this._openArchive(context).then((context) => {
                context = new ModelContext(context);
                var extension = context.identifier.split('.').pop().toLowerCase();
                var modules = this._filter(context);
                if (modules.length == 0) {
                    reject(new ModelError("Unsupported file extension '." + extension + "'."));
                    return;
                }
                var errors = [];
                var matches = 0;
                var nextModule = () => {
                    if (modules.length > 0) {
                        ...
                        this._host.require(id).then((module) => {
                            if (!module.ModelFactory) {
                                reject(new ModelError("Failed to load module '" + id + "'."));
                                return;
                            }
                            ...
                            matches++;
                            modelFactory.open(context, this._host).then((model) => {
                                resolve(model);
                            }).catch((error) => {
                                errors.push(error);
                                nextModule();
                        ...
                        if (matches > 0) {
                            ...
                            reject(new ModelError(errors.map((err) => err.message).join('\n')));
                            return;
                        }
                        reject(new ModelError("Unsupported file content for extension '." + extension + "' in '" + context.identifier + "'."));
                        return;
                    }
                };
                nextModule();
            ...
            return new Promise((resolve, reject) => {
                var nextEntry = () => {
                    if (entries.length > 0) {
                            ...
                            if (identifier.length > 0 && identifier.indexOf('/') < 0 && !identifier.startsWith('.')) {
                                ...
                                var nextModule = () => {
                                    if (modules.length > 0) {
                                        ...
                                        this._host.require(id).then((module) => {
                                            if (!module.ModelFactory) {
                                                reject(new ArchiveError("Failed to load module '" + id + "'.", null));
                                            }
                        ...
                        if (matches.length == 0) {
                            reject(new ArchiveError('Root does not contain model file.'));
                            return;
                        }
                        else if (matches.length > 1) {
                            reject(new ArchiveError('Root contains multiple model files.'));
                            return;
                        }
                        var match = matches[0];
                        resolve(new ModelContext(new ArchiveContext(entries, rootFolder, match.name, match.data)));
                        return;
                    }
                };
                nextEntry();
